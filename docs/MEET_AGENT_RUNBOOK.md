...
**Завершение встречи (драйвер, без DOM-счётчика):** participantCount парсится из
DOM ненадёжно (бывает None) → драйвер определяет конец по ПРИЗНАКАМ ИЗ ДИАЛОГА:
после прощания (`_is_farewell_candidate`) ждёт `DRIVER_END_GRACE`=25с тишины людей →
завершает (graceful → авто-отчёт); бэкстоп — `DRIVER_MAX_IDLE`=300с без речи людей.
Не зависит от `_detect_alone`/participantCount.

**Address-gating:** в живом диалоге драйвер реагирует ТОЛЬКО когда к боту обратились
по имени (wake-word «Вертер»/«Verter», `_WAKE`) — не влезает в разговор людей между
собой. ИСКЛЮЧЕНИЯ (без обращения): приветствие при входе и прощание перед выходом
(детект `_is_farewell_candidate` → договаривает прощание до отключения). Тяжёлые
фоновые задачи идут через tool-capable `hermes -z` (web/файлы/команды), быстрые
реплики — через тёплый `call_llm`.
